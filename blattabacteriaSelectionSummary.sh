...
grep "SELECTED SITES" blattabacteriaQSD.out | cut -f3 -d " " | paste -d " " - - | sed "s/NO/0/" | awk 'BEGIN{print "QSDnegative\tQSDpositive"}1' | paste blattabacteriaGeneSummary.csv - > temp
